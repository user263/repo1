...
Функция РассчитатьСтоимостиРемонтов(ПланРемонтов, Ссылка, МассивДоступныхДляКорректировкиСтрок, ТекущаяСтрока = Неопределено) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ТабНоменклатуры.Номенклатура,
	               |	ТабНоменклатуры.ЕдиницаИзмерения,
	               |	ТабНоменклатуры.Количество,
	               |	ТабНоменклатуры.ОбъектРемонтныхРабот,
	               |	ТабНоменклатуры.ВидРемонта,
	               |	ТабНоменклатуры.ID
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'План-график регламентных мероприятий'");
...
Функция ПечатьПланГрафикРегламентныхМероприятий(МассивОбъектов, ПараметрыПечати)
	...
	Если (ПланГрафикРемонта = Неопределено) тогда
		торо_ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не выбран документ ""График регламентных мероприятий"".'"));
	Иначе
