...
Процедура ИспользоватьИнтеграциюС1СДокументооборотПриИзменении(Элемент)
	...
	УстановитьСопутствующиеКонстанты();
...
&НаСервере 
Процедура  УстановитьСопутствующиеКонстанты()
	Если НаборКонстант["ИспользоватьИнтеграциюС1СДокументооборот"] = Ложь Тогда
		Константы.ИспользоватьПроцессыИЗадачи1СДокументооборота.Установить(Ложь);
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьНастройкиОбновленияСвязанныхРеквизитов(Расписание = Неопределено)
	...
	Задание = РегламентныеЗадания.НайтиПредопределенное(Метаданные.РегламентныеЗадания.ИнтеграцияС1СДокументооборотВыполнитьОбменДанными);
