...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Если ПараметрыМетода.ИскатьПомеченные И ПараметрыМетода.УдалятьПомеченные Тогда
		Состояние(НСтр("ru = 'Поиск и удаление помеченных объектов...'"));
	ИначеЕсли ПараметрыМетода.ИскатьПомеченные Тогда
	...
	ИначеЕсли Задание.ОшибкаПриУстановкеМонопольногоРежима Тогда
		...
		Иначе
			...
			ПараметрыПредупреждения.Текст    = НСтр("ru = 'Не удалось установить монопольный режим'");
			ПараметрыПредупреждения.Подробно = Задание.ТекстОшибкиУстановкиМонопольногоРежима;
			СтандартныеПодсистемыКлиент.ВывестиПредупреждение(
				ЭтотОбъект,
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	Если ПараметрыМетода.УдалятьПомеченные И Монопольно Тогда
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда
			...
			Результат.Вставить("ТекстОшибкиУстановкиМонопольногоРежима", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
