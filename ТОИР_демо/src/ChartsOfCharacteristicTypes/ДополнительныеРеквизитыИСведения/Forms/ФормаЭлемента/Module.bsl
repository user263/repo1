...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоДопРеквизитОР = ПроверитьЭтоДопРеквизитОР(НаборСвойствДляПроверки);
	// ТОиР--
...
Процедура ПометитьНаУдалениеСнятьПометку(Команда)
	...
	Если НЕ Объект.ПометкаУдаления Тогда
		...
		ТекстОповещенияПользователя = НСтр("ru ='Пометка удаления установлена'");
	Иначе
		ШаблонВопроса = НСтр("ru ='Снять с ""%1"" пометку на удаление?'");
		ТекстОповещенияПользователя = НСтр("ru ='Пометка удаления снята'");
	КонецЕсли;
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонВопроса, Объект.Заголовок);
	
	// Платформенный вопрос модальный, поэтому тут тоже делаю модально.
	...
	ПоказатьОповещениеПользователя(ТекстОповещенияПользователя, ПолучитьНавигационнуюСсылку(Объект.Ссылка), Объект.Заголовок, БиблиотекаКартинок.Информация32);
...
&НаСервере
Процедура ПометитьНаУдалениеСнятьПометкуНаСервере()
	
	Если НЕ Объект.Ссылка.Пустая() Тогда
		ОбъектОбъект = РеквизитФормыВЗначение("Объект");
		ОбъектОбъект.ПометкаУдаления = НЕ Объект.ПометкаУдаления;
		ОбъектОбъект.Записать();
				
		Если ЗначениеЗаполнено(Объект.НаборСвойств) Тогда
			НаборОбъект = Объект.НаборСвойств.ПолучитьОбъект();
			СтрокаРеквизита = НаборОбъект.ДополнительныеРеквизиты.Найти(Объект.Ссылка);
			Если СтрокаРеквизита <> Неопределено Тогда
				СтрокаРеквизита.ПометкаУдаления = ОбъектОбъект.ПометкаУдаления;
