...
Функция ИмяФайлаНастроекДляПриемника() Экспорт
	...
	Возврат НСтр("ru = 'Настройки синхронизации для EAM ERP'");
...
Функция НастройкаОтборовНаУзле(ВерсияКорреспондента, ИмяФормы, ИдентификаторНастройки) Экспорт
	...
	СтруктураТабличнойЧастиКритичностей = Новый Структура;
	СтруктураТабличнойЧастиКритичностей.Вставить("КритичностьДефекта", Новый Массив);
	СтруктураТабличнойЧастиКритичностей.Вставить("ВремяНаУстранение" , Новый Массив);

	
	СтруктураНастроек = Новый Структура;
	СтруктураНастроек.Вставить("ИспользоватьОтборПоОрганизациям", Ложь);
	СтруктураНастроек.Вставить("ПереноситьАктивыВУпп"           , Истина);
	СтруктураНастроек.Вставить("ОтражатьПринятиеКУчетуОСВУПП"   , Истина);
	СтруктураНастроек.Вставить("ОтражатьСписаниеОСВУПП"         , Ложь);
	СтруктураНастроек.Вставить("ОтражатьВыявленныеДефекты"      , Истина);
	СтруктураНастроек.Вставить("ОтражатьЗаказыНаРемонт"         , Истина);
	СтруктураНастроек.Вставить("РемонтыПоППР"                   , Истина);
	СтруктураНастроек.Вставить("РемонтыПоДефектам"              , Истина);
	СтруктураНастроек.Вставить("РемонтыПоВнешнимОснованиям"     , Истина);
	...
	СтруктураНастроек.Вставить("РежимВыгрузкиПоУсловию"         , Перечисления.РежимыВыгрузкиОбъектовОбмена.ВыгружатьПоУсловию);
	...
	СтруктураНастроек.Вставить("ТаблицаСоответствияКритичностиИВремениУстранения", СтруктураТабличнойЧастиКритичностей);
	СтруктураНастроек.Вставить("ВыгружатьПоступлениеИВнутреннееПотребление"     , Истина);
...
Функция ОписаниеОграниченийПередачиДанных(НастройкаОтборовНаУзле, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	Если Не НастройкаОтборовНаУзле.ПереноситьАктивыВУпп Тогда
		СтрокаПереносаОбъектовРемонта = НСтр("ru = 'Перенос объектов ремонта:
													|- объекты ремонта-активы не переносить.'");
	Иначе
		
		СтрокаПереносаОбъектовРемонта = НСтр("ru = 'Перенос объектов ремонта:
													|- переносить объекты ремонта-активы[РасшифровкаПереноса].'");
		
		Если НастройкаОтборовНаУзле.ОтражатьПринятиеКУчетуОСВУПП = Ложь
			И НастройкаОтборовНаУзле.ОтражатьСписаниеОСВУПП = Ложь Тогда
			
			РасшифровкаПереноса = НСтр("ru = ' без создания документов Принятия к учету ОС и Списания ОС в ERP'");
			
		ИначеЕсли НастройкаОтборовНаУзле.ОтражатьПринятиеКУчетуОСВУПП = Истина
			И НастройкаОтборовНаУзле.ОтражатьСписаниеОСВУПП = Ложь Тогда
			
			РасшифровкаПереноса = НСтр("ru = ', принятые к учету в ТОиР, с созданием документов ""Принятие к учету ОС"" в ERP'");		
			
		ИначеЕсли НастройкаОтборовНаУзле.ОтражатьПринятиеКУчетуОСВУПП = Истина
			И НастройкаОтборовНаУзле.ОтражатьСписаниеОСВУПП = Истина Тогда
			
			РасшифровкаПереноса = НСтр("ru = ', принятые к учету в ТОиР или списанные документами ""Выбытие ОР""'");
			
		КонецЕсли;
		
		Результат = СтрЗаменить(СтрокаПереносаОбъектовРемонта, "[РасшифровкаПереноса]", РасшифровкаПереноса) + Символы.ПС; 
	...
	// перенос ремонтов
	Если Не НастройкаОтборовНаУзле.ОтражатьЗаказыНаРемонт Тогда
		СтрокаПереносаОбъектовРемонта = НСтр("ru = 'Перенос цепочек ремонтов:
													|- не переносить.");
	Иначе
		...
		Если НастройкаОтборовНаУзле.РемонтыПоППР Тогда
			
			РасшифровкаПереноса = РасшифровкаПереноса + НСтр("ru = '- ремонты по ППР
																|'");
		КонецЕсли;	
		
		Если НастройкаОтборовНаУзле.РемонтыПоДефектам Тогда
			РасшифровкаПереноса = РасшифровкаПереноса + НСтр("ru = '- ремонты по выявленным дефектам
																|'");
		КонецЕсли;	
		
		Если НастройкаОтборовНаУзле.РемонтыПоВнешнимОснованиям Тогда
			
			РасшифровкаПереноса = РасшифровкаПереноса + НСтр("ru = '- ремонты по внешним основаниям
																|'");
			
		КонецЕсли;
		
		Результат = Результат + "
		|" + СтрЗаменить(СтрокаПереносаОбъектовРемонта, "[РасшифровкаПереноса]", РасшифровкаПереноса) + Символы.ПС; 
		
	КонецЕсли; 
	
	// перенос дефектов
	Если НастройкаОтборовНаУзле.ОтражатьВыявленныеДефекты Тогда
		
		РасшифровкаПереноса = НСтр("ru = 'Переносить документы <Выявленные дефекты> как <Регистрации дефектов> ([ПараметрыЗаполненияКритичностей]).'");
		
		Если НастройкаОтборовНаУзле.ТаблицаСоответствияКритичностиИВремениУстранения.Количество() > 0 Тогда
			ПараметрыЗаполненияКритичностей = "настройки критичности выполнены";
		Иначе
			ПараметрыЗаполненияКритичностей = "настройки критичности не выполнены";
		КонецЕсли; 
		
		Результат = Результат + СтрЗаменить(РасшифровкаПереноса, "[ПараметрыЗаполненияКритичностей]", ПараметрыЗаполненияКритичностей);
	Иначе
		Результат = Результат + НСтр("ru = 'Не переносить документы Выявленные дефекты отдельно.'");
	КонецЕсли; 
...
Функция ЗаголовокКомандыДляСозданияНовогоОбменаДанными() Экспорт
	...
	Возврат НСтр("ru = 'Обмен с конфигурацией 1С: ERP Управление предприятием, редакция 2.0'");
...
Функция КраткаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	ПоясняющийТекст = НСтр("ru = 'Данная настройка позволит синхронизировать данные между программами 1С: ТОИР Управление ремонтами и обслуживанием оборудования 2.0 и 1С: ERP Управление предприятием, редакция 2.0.'");
...
Функция ПодробнаяИнформацияПоОбмену(ИдентификаторНастройки) Экспорт
	...
	Иначе
		...
		Возврат "ПланОбмена.ОбменТОиР20ЕРП20.Форма.ПодробнаяИнформация";
...
Функция ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзле, ВерсияКорреспондента,ИдентификаторНастройки) Экспорт
	...
	Возврат НСтр("ru = 'Получение данных по всем выгруженным из 1С: ТОИР Управление ремонтами и обслуживанием оборудования 2.0 объектам ремонта
			|и принятым к учету в 1С: ERP Управление предприятием, редакция 2.0.'");
...
// Дополнительно можно назначить переменной Сообщение описание для пользователя, 
