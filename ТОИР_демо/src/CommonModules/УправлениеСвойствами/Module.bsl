...
Процедура ПриСозданииНаСервере(Форма, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если НЕ Контекст.ПроизвольныйОбъект 
		...
		ДобавитьНаФормуТаблицуЗначенийДополнительныхРеквизитов(Форма, Контекст);
		// Доп. реквизиты в таблице--
	КонецЕсли;
			...
			Если Строка.ЗаполнятьОбязательно И Строка.АвтоОтметкаНезаполненного И НЕ Строка.Удалено Тогда
				Если НЕ ЗначениеЗаполнено(Строка.Значение) Тогда
					...
					ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
						"Свойства_ЗначенияДополнительныхРеквизитов["+Индекс+"].Значение",
...
Процедура ЗаполнитьДополнительныеРеквизитыВФорме(Форма, Объект = Неопределено, ПоляНадписей = Ложь, СкрытьУдаленные = Неопределено, ИмяРеквизитаНаборСвойств = "") Экспорт
	...
	Для Каждого ОписаниеСвойства Из ОписаниеСвойств Цикл
		...
		// Доп. реквизиты в таблице++
		Если НЕ ОтображатьЗначенияВТаблице Тогда
		// Доп. реквизиты в таблице--	
	...
	// Доп. реквизиты в таблице--	
		...
		Для Каждого ОписаниеЗависимогоРеквизита Из Форма.Свойства_ОписаниеЗависимыхДополнительныхРеквизитов Цикл
			...
			Если ОписаниеЗависимогоРеквизита.УсловиеОбязательностиЗаполнения <> Неопределено Тогда
				...
				Элемент = Форма.Элементы[ОписаниеЗависимогоРеквизита.ИмяРеквизитаЗначение];
				Если Элемент.Вид = ВидПоляФормы.ПолеВвода И Элемент.АвтоОтметкаНезаполненного <> Результат Тогда
...
// Доп. реквизиты в таблице++
Процедура ДобавитьНаФормуТаблицуЗначенийДополнительныхРеквизитов(Форма, Контекст)
...
Процедура ЗаписатьДопРеквизитыСведенияНабора(Форма, Отказ, ТипЭлемента, НаборСвойствДопРеквизитовСведений, ИмяТаблицыДопРеквизитовСведений = "") Экспорт
	...
	Исключение
		...
		ИмяСобытия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ИмяСобытия,	ТипСведений);
		...
		ВыражениеИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ВыражениеИсключения,	ТипСведений);
