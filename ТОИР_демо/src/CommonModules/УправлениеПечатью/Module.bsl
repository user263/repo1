...
Процедура ПриСозданииНаСервере(Форма, МестоРазмещенияКомандПоУмолчанию = Неопределено, Знач ОбъектыПечати = Неопределено) Экспорт
	...
	Если ОбъектыПечати <> Неопределено Тогда
		...
		Для Каждого ОбъектМетаданных Из ОбъектыПечати Цикл
			СписокОбъектов.Добавить(ОбъектМетаданных.ПолноеИмя());
		КонецЦикла;
		СписокОбъектов = ОбщегоНазначения.ЗначениеВСтрокуXML(СписокОбъектов);
	КонецЕсли;
...
//  ИсточникиКоманд - Массив - список объектов метаданных, из которых необходимо брать команды печати.
//
// Возвращаемое значение:
//  ТаблицаЗначений - описание см. в СоздатьКоллекциюКомандПечати().
Функция КомандыПечатиФормы(Форма, ИсточникиКоманд = Неопределено) Экспорт
	
	СписокОбъектов = Неопределено;
	Если ТипЗнч(ИсточникиКоманд) = Тип("Массив") И ИсточникиКоманд.Количество() > 0 Тогда
		Если ТипЗнч(ИсточникиКоманд[0]) = Тип("Строка") Тогда
			СписокОбъектов = Новый Массив;
			Для Каждого ИмяОбъектаМетаданных Из ИсточникиКоманд Цикл
				СписокОбъектов.Добавить(Метаданные.НайтиПоПолномуИмени(ИмяОбъектаМетаданных));
			КонецЦикла;
		Иначе
			СписокОбъектов = ИсточникиКоманд;
		КонецЕсли;
	КонецЕсли;
