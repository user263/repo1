...
// Параметры:
//	УИДЗамера - УникальныйИдентификатор - уникальный идентификатор замера.
//	ВесЗамера - Число(10,0)				- количественный показатель сложности
...
// Параметры:
//	УИДЗамера	- УникальныйИдентификатор	- уникальный идентификатор замера.
// 	Комментарий - Строка(900), Соответствие	- дополнительная произвольной информации замера.
//
...
//
// При проведении замеров производительности ключевые операции разделяются на "Хорошие"
// и "Плохие". Для примера, если документ при проведении прошел все проверки, не возникло исключений,
// то это "Хорошая" ключевая операция. Если же документ при проведении не прошел проверку заполнения,
// то это "Плохая" ключевая операция, т.е. не выполнялся весь программный код при проведении и такой
// замер может быть намного меньше "Хорошего".
//
// Например,
// &НаКлиенте
// Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
// 	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
//		ИдентификаторЗамераПроведение = ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "_ДемоПроведениеДокументаОшибка");
...
//	УникальныйИдентификатор - уникальный идентификатор замера, который позволяет идентифицировать замер.
//
Функция НачатьЗамерВремениСоСмещением(Смещение, АвтоЗавершение = Истина, КлючеваяОперация = Неопределено) Экспорт
...
Процедура ЗаписатьРезультатыАвтоНеГлобальный(ПередЗавершением = Ложь) Экспорт
	...
	Если ОценкаПроизводительностиЗамерВремени <> Неопределено Тогда
		...
		АдресХранилища = ПоместитьВоВременноеХранилище(ЗамерыДляЗаписи, Новый УникальныйИдентификатор("cd097b4b-fcda-4ac8-bf4f-f99e42ed928c")); 
