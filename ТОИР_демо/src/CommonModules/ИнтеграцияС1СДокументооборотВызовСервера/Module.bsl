...
Функция ПроверитьПодключение(Знач АдресСервиса = Неопределено, Знач ИмяПользователя, Знач Пароль,
	...
	// Вызовем конструкторы через Выполнить, чтобы обеспечить совместимость с версиями платформы
	// до 8.3.6, не поддерживающими аутентификацию ОС при работе с веб-сервисами.
	
	Если ПоддерживаетсяАутентификацияОС
		И ИспользуетсяАутентификацияОС Тогда
		ПараметрАутентификацияОС = ", Истина";
	Иначе
		ПараметрАутентификацияОС = "";
	КонецЕсли;
	...
	Если Определения = Неопределено Тогда
		...
		Попытка
			...
			Выполнить(ВызовКонструктора); // Создание WSОпределения по ссылке ws/DMService?wsdl
	...
	Попытка
		...
		Выполнить(ВызовКонструктора); // Создание WSПрокси
	...
	Если ПоддерживаетсяАутентификацияОС Тогда
		ИспользуетсяАутентификацияОС = Прокси.ИспользоватьАутентификациюОС;
	Иначе
		ИспользуетсяАутентификацияОС = Ложь;
	КонецЕсли;
	ПараметрыСеанса.ИнтеграцияС1СДокументооборотИспользуетсяАутентификацияОС = ИспользуетсяАутентификацияОС;
...
Функция ИспользоватьПрисоединенныеФайлы1СДокументооборотаДляОбъекта(ОбъектИС) Экспорт
	...
	ИмяСправочника = МодульПрисоединенныеФайлыСлужебный.ИмяСправочникаХраненияФайлов(ОбъектИС);
	ПолноеИмяСправочника = "Справочник." + ИмяСправочника;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ИмяСправочника", ПолноеИмяСправочника);
	Запрос.УстановитьПараметр("ВладелецФайлов", ОбъектИС);
