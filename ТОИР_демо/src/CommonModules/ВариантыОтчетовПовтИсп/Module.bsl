	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		Если ПравоДоступа("Просмотр", ОтчетМетаданные)
			...
			Если ПроверятьФункциональныеОпции
				...
				Продолжить;
			КонецЕсли;
			СсылкаОтчета = СтандартныеПодсистемыСервер.ИдентификаторОбъектаМетаданных(ОтчетМетаданные);
			Если ТипЗнч(СсылкаОтчета) = Тип("Строка") Тогда
				Продолжить;
			КонецЕсли;
			Результат.Добавить(СсылкаОтчета);
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьПодсистемыТекущегоПользователя(РодительСтрока, РодительМетаданные = Неопределено, РазделСсылка = Неопределено)
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		КонецЕсли;
		Если ТипЗнч(ПодсистемаСсылка) = Тип("Строка") Тогда
			Продолжить;
		КонецЕсли;
		...
		КонецЕсли;
		Если ДляКорневойСтроки Тогда
			СтрокаДерева.Ссылка              = ПодсистемаСсылка;
