...
Процедура ЗарегистрироватьТипыИСвязи(Информация, УзелКД, ЭлементКД, НастройкаВарианта)
	...
	Для Каждого Тип Из ИнформацияОТипах.ОбъектныеТипы Цикл
		...
		Если ДляВыбораЭтогоТипаИспользуетсяБыстрыйВыбор Тогда
			...
			ФрагментЗапросаИз = " ИЗ " + ПолноеИмя + Символы.ПС;
			Если ТекстЗапросаСсылок = "" Тогда
				ТекстЗапросаСсылок = "ВЫБРАТЬ РАЗЛИЧНЫЕ РАЗРЕШЕННЫЕ Ссылка" + ФрагментЗапросаИз;
			ИначеЕсли СтрНайти(ТекстЗапросаСсылок, ФрагментЗапросаИз) = 0 Тогда
				ТекстЗапросаСсылок = ТекстЗапросаСсылок + Символы.ПС
					+ "ОБЪЕДИНИТЬ ВСЕ" + Символы.ПС + Символы.ПС
					+ "ВЫБРАТЬ РАЗЛИЧНЫЕ Ссылка" + ФрагментЗапросаИз;
			КонецЕсли;
		КонецЕсли;
	...
	Если СвойстваНастройки.ТипЭлементов <> "" Тогда
		...
		ИначеЕсли СвойстваНастройки.ТипЭлементов = "СвязьСКомпоновщиком" Тогда
			...
			Если СвойстваНастройки.Тип = "ЗначениеПараметраНастроек"
				...
				// Для платформы (увеличение размера поля типа 'Дата' чтобы вмещались варианты выбора 'СтандартнаяДатаНачала').
