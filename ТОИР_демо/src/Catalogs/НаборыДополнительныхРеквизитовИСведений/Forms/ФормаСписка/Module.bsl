...
Процедура НастроитьОтображениеНаборов()
	...
	// ТОиР
	ОбновитьСписокНаборовТолькоДляПросмотра();
	// ТОиР
...
Процедура СвойстваУстановитьДоступностьКоманд(Контекст)
	...
	// ТОиР
	НаборыТолькоДляПросмотра = Контекст.НаборыТолькоДляПросмотра;
	// ТОиР
	...
	// ТОиР
	ДоступностьРедактированияНабора = Истина;
	Если ЗначениеЗаполнено(Контекст.ТекущийНабор) 
		И НаборыТолькоДляПросмотра.НайтиПоЗначению(Контекст.ТекущийНабор) <> Неопределено Тогда
		ДоступностьРедактированияНабора = Ложь;
	КонецЕсли;
	// ТОиР

	// Настройка команд командной панели.
	Элементы.ДобавитьИзНабора.Доступность           = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваСоздать.Доступность            = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваТолькоСоздать.Доступность      = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваСоздатьПоОбразцу.Доступность   = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваСоздатьОбщее.Доступность       = ОбщаяДоступность И ДоступностьРедактированияНабора;
	
	Элементы.СвойстваСкопировать.Доступность        = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.СвойстваИзменить.Доступность           = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.СвойстваПометитьНаУдаление.Доступность = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	
	Элементы.СвойстваПереместитьВверх.Доступность   = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.СвойстваПереместитьВниз.Доступность    = ДоступностьДляСтроки И ДоступностьРедактированияНабора;

	
	Элементы.КопироватьРеквизит.Доступность         = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.ВставитьРеквизит.Доступность           = ДоступностьВставки И ДоступностьРедактированияНабора;
	
	// Настройка команд контекстного меню.
	Элементы.СвойстваКонтекстноеМенюСоздать.Доступность            = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваКонтекстноеМенюТолькоСоздать.Доступность      = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваКонтекстноеМенюДобавитьИзНабора.Доступность   = ОбщаяДоступность И ДоступностьРедактированияНабора;
   Элементы.СвойстваКонтекстноеМенюСоздатьПоОбразцу.Доступность   = ОбщаяДоступность И ДоступностьРедактированияНабора;
	Элементы.СвойстваКонтекстноеМенюСоздатьОбщее.Доступность       = ОбщаяДоступность И ДоступностьРедактированияНабора;
	
	Элементы.СвойстваКонтекстноеМенюСкопировать.Доступность        = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.СвойстваКонтекстноеМенюИзменить.Доступность           = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.СвойстваКонтекстноеМенюПометитьНаУдаление.Доступность = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	
	Элементы.СвойстваКонтекстноеМенюКопироватьРеквизит.Доступность = ДоступностьДляСтроки И ДоступностьРедактированияНабора;
	Элементы.СвойстваКонтекстноеМенюВставитьРеквизит.Доступность   = ДоступностьВставки И ДоступностьРедактированияНабора;
	
	Элементы.Свойства.ТолькоПросмотр = НЕ ДоступностьРедактированияНабора;
...
// ТОиР
&НаСервере
Процедура ОбновитьСписокНаборовТолькоДляПросмотра()
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	торо_НаправленияОбъектовРемонтныхРабот.НаборСвойствОР КАК НаборСвойств
	|ИЗ
	|	Справочник.торо_НаправленияОбъектовРемонтныхРабот КАК торо_НаправленияОбъектовРемонтныхРабот
	|ГДЕ
	|	торо_НаправленияОбъектовРемонтныхРабот.НаборСвойствОР <> ЗНАЧЕНИЕ(Справочник.НаборыДополнительныхРеквизитовИСведений.ПустаяСсылка)";
	
	Наборы = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("НаборСвойств");
	НаборыТолькоДляПросмотра.ЗагрузитьЗначения(Наборы);
